<template>
  ...
  <div class="alert alert-dark" role="alert" v-if="table.successIzin">
    <b>"Proses Izin"</b> telah selesai diproses dengan rincian : <b>({{ table.logIzin?.send }} Data)</b> Dikirim | <b class="text-white">({{ table.logIzin?.process }} Data)</b> Diproses |
    ...
    <div :class="auth?.user?.role != 'sysadmin' && auth?.user?.role != 'admin' ? 'col-md-6' : 'col-md-4'" v-if="access.wilayah()">
      ...
      <div class="filter-box row">
        <div class="col-md-4" v-if="access.notInternal()">
          <select class="form-select form-select-sm mb-2" v-model="table.paramsSurat.wilayah" @change="table.getBlokSurat">
            <option value="" selected>Semua Wilayah</option>
            <option v-for="w in table.filterSurat.wilayah" :key="w" :value="w.alias_wilayah">
              {{ w.wilayah }}
            </option>
          </select>
          <select class="form-select form-select-sm mb-2" :disabled="table.paramsSurat.wilayah === ''" v-model="table.paramsSurat.blok" @change="table.getDataSurat">
            <option value="" selected>Semua Daerah</option>
            <option v-for="b in table.filterSurat.blok" :key="b" :value="b.id_blok">
              {{ b.blok }}
            </option>
          </select>
        </div>
        <div class="col-md-4" v-if="access.notInternal()">
          <select class="form-select form-select-sm mb-2" v-model="table.paramsSurat.area" @change="table.getDropSurat">
            <option value="" selected>Semua Area</option>
            <option v-for="a in table.filterSurat.area" :key="a" :value="a.id">
              {{ a.nama }}
            </option>
          </select>
          <select class="form-select form-select-sm mb-2" v-model="table.paramsSurat.dropspot" @change="table.getDataSurat" :disabled="table.paramsSurat.area === ''">
            <option value="" selected>Semua Dropspot</option>
            <option v-for="d in table.filterSurat.dropspot" :key="d" :value="d.id">
              {{ d.nama }}
            </option>
          </select>
          <select class="form-select form-select-sm mb-2" v-model="table.paramsSurat.cetak" @change="table.getDataSurat">
      ...
      <div class="serach-box row mt-2 mb-3">
        <div class="col-md-6 d-flex">
          ...
          <div class="col-auto me-3">
            <div class="form-control-plaintext form-control-sm">Total data {{ table.metaSurat["x_total_data"] }}</div>
          </div>
          <div class="col-auto">
            <button class="btn btn-sm btn-primary" @click="aktifMemilih = !aktifMemilih"><font-awesome-icon icon="check-square" /> Aktifkan Memilih</button>
          </div>
        </div>
        <div class="col-md-6">
          <div class="row justify-content-end">
            <div class="col-md-4">
              <input type="text" v-model="table.paramsSurat.cari" class="form-control form-control-sm" placeholder="Cari ..." @update:model-value="table.getDataSurat" />
            </div>
          </div>
        </div>
      </div>
          ...
          <thead>
            <tr>
              <th v-if="aktifMemilih">
                <input type="checkbox" :checked="semuaTerpilih" @click="pilihSemua" />
              </th>
              <th>No</th>
              <th>NIUP</th>
              <th>Nama</th>
          ...
          <tbody>
            <tr v-for="(d, i) in table.itemsSurat">
              <td v-if="aktifMemilih">
                <input type="checkbox" :id="d.id" :value="d.persyaratan.id" v-model="itemDipilih" />
              </td>
              <td>{{ i + 1 }}</td>
              <td>{{ d?.santri?.niup }}</td>
              <td>{{ d?.santri?.nama_lengkap }}</td>
  ...
  <div class="modal fade" v-if="table.openLogin === true" :class="{ show: table.openLogin }" style="display: block" id="modalEdit" tabindex="-1" aria-labelledby="modalEditLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="modalEditLabel">PEDATREN</h1>
          <button class="btn-close" type="button" @click="table.setOpenLogin"></button>
        </div>
        <form @submit.prevent="table.loginPedatren(auth.user.uuid)">
          <div class="modal-body">
            <div class="text-center">
              <img src="https://nuruljadid.app/img/logo.faa9427b.png" alt="pedatren-logo" class="mb-2" />
              <h3>PEDATREN</h3>
              <p>(Pengelolaan Data Pesantren)</p>
            </div>
            <div class="form-group mb-3">
              <input type="text" class="form-control mt-2" placeholder="Username" v-model="table.form.username" />
            </div>
            <div class="form-group mb-3">
              <input type="password" class="form-control mt-2" placeholder="Password" v-model="table.form.password" />
            </div>
            <button type="submit" class="btn w-100 btn-sm btn-info">Login</button>
          </div>
        </form>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-sm btn-secondary" @click="table.setOpenLogin">Batal</button>
        </div>
      </div>
    </div>
  </div>
  <!-- View Log -->
  <div class="modal fade modal-xl" v-if="table.openLog === true" :class="{ show: table.openLog }" style="display: block" id="modalEdit" tabindex="-1" aria-labelledby="modalEditLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="modalEditLabel">Log Error PEDATREN</h1>
          <button class="btn-close" type="button" @click="table.setOpenLog"></button>
        </div>
        <div class="modal-body">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-sm btn-secondary" @click="table.setOpenLog">Batal</button>
        </div>
      </div>
    </div>
  </div>
...
const notSync = auth.user.username_pedatren === null && auth.user.password_pedatren === null;
...
`,
    ...
    // Tambahkan paragraf
    doc.text(`Yang bertanda tangan dibawah ini, Kepala Pondok Pesantren Nurul Jadid Paiton Probolinggo Jawa Timur `, 1, 4.8);
